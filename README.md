...
| 項目 | 優先度 | 由来 | 概要 |
|---|---|---|---|
| **構造化出力の強制 (Ollama `format=json`)** | 高 | m1/m4/rp6 JSON崩れ | Plannerプロンプトで「\\n でエスケープ」と指示してもLLMが従わないことがある。Ollama API の `format=json` パラメータで JSON 構文を強制する |
| **code 引数のファイルパス化** | 高 | m1/m4 JSON崩れ | `code_executor` の args に長いコード文字列を入れる現状が JSON escape 失敗の温床。先に file_ops で `.py` を書き、code_executor は path を受ける形に変更 |
| **Verifier コンポーネント独立化** | 中 | replan rp4 偽done | Observerから「結果の妥当性検証」責務を分離して `Verifier` クラスにし、タスク達成の検証ロジックを独立してテスト・改善可能にする |
| **MemorySearchTool 化** | 中 | Phase 2 検討事項 | 3層メモリの L3 検索を `BaseTool` 化し、Planner が必要時のみ明示呼び出しできるようにする (Phase 2 では透過注入方式を採用) |
...
| **WhisperAdapter (音声入力)** | 中 | README Phase 3 予定 | faster-whisper で音声→テキスト変換、CLIに音声入力モード追加 |
