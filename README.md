...
| **MemorySearchTool** | 中 | 3層メモリの L3 検索を `BaseTool` 化し、Planner が必要時のみ明示呼び出しできるようにする (Phase 2 では透過注入方式を採用、本項目は次フェーズの選択肢) |
| **専用OCRツール (Tesseract / PaddleOCR)** | 中 | Gemma 3 のネイティブビジョンは物体認識・短文抽出は得意だが、長文・表・日本語縦書き・手書きは精度が落ちる。`BaseTool` として OCR エンジンを追加し、VisionAdapter から委譲できるようにする (Phase 2 では Gemma 3 単体でカバー) |
