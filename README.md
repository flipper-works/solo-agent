...
│  ┌───────────────┐ ┌──────────────┐ ┌───────────────┐  │
│  │ テキスト用     │ │ Vision 用    │ │ 音声用        │  │
│  │ gemma3-sp     │ │ (gemma3 内蔵)│ │ faster-whisper│  │
│  │ (常駐)        │ │ (Phase 2)    │ │ (Phase 3)     │  │
│  └───────────────┘ └──────────────┘ └───────────────┘  │
└────────────────────────────────────────────────────────┘
```
...
| レイヤー | 技術 | 選定理由 |
|---------|------|---------|
| **LLM** | Ollama + gemma3-sp (Gemma 3 12B) | ローカル実行、マルチモーダル (テキスト+画像)、日本語対応、Google 製 OSS |
...
| 用途 | 推奨モデル | VRAM目安 | 導入状況 |
|------|-----------|---------|------------|
| 汎用・デフォルト | gemma3-sp (Gemma 3 12B + System Prompt) | ~8GB | 実装済 |
| コード特化 | gemma3-sp (同上、コード生成も良好) | ~8GB | 実装済 |
| 日本語特化サブ | swallow:8b-v0.5 (Llama-3.1-Swallow-8B) | ~5GB | 実装済 |
| 画像理解 | gemma3:12b (ネイティブビジョン) | ~8GB | 実装済 |
| 音声認識 | faster-whisper (small/medium/large-v3) | 0.5〜3GB | 実装済 |
...
| VisionAdapter | gemma3:12b ネイティブビジョンで画像→テキスト記述 | Phase 2 (実装済) | スクショデバッグ・ER図読み込み |
| WhisperAdapter | faster-whisper (GPU対応、34倍速) でローカル音声→テキスト | Phase 3 (実装済) | 口頭タスク指示 |
...
│       │   ├── executor.py     # タスク実行制御
│       │   ├── observer.py     # 結果評価・判断
│       │   ├── verifier.py     # ★ タスク達成の検証 (Observerから分離)
│       │   └── session.py      # セッション管理
│       │
│       ├── eval/               # ★ 評価ハーネス
│       │   ├── runner.py       # baseline / replan 評価
│       │   ├── multiturn.py    # multi-turn 会話評価
│       │   └── grader.py       # LLM-as-judge 自動採点
│       │
│       ├── training/           # ★ FT データパイプライン
│       │   ├── schema.py       # SFTRecord (ChatML形式)
│       │   ├── builder.py      # dedupe / split / stats
│       │   ├── exporter.py     # JSONL 出力
│       │   └── sources/
│       │       ├── curated.py  # YAML手動データ
│       │       ├── episodes.py # ChromaDB → SFT
│       │       └── augment.py  # LLM自動増強
│       │
│       ├── mcp_server.py       # ★ MCP サーバー (stdio transport)
│       │
...
### 5.3.1 Verifier の責務 (Observerから分離)

- LLM-as-judge でタスクの「真の達成」を厳しく検証する独立コンポーネント
- V1〜V6 のチェックリストで done 判定の前に妥当性確認
- BaseVerifier Protocol により、ルールベース / FT専用モデル等に差し替え可能
- Observer は Verifier の結果を受けて verdict (done/replan/fail) を決定論的に決める
...
Week 9: マルチモーダル — 画像入力
  ✓ gemma3:12b ネイティブビジョン統合 & 遅延ロード管理
...
Week 10-11: マルチモーダル — 音声入力
  ✓ faster-whisper (GPU対応、34倍速) ローカル統合
...
| Vision/faster-whisper モデル未ロード | テキストフォールバック + ユーザーへの通知 |
...
# System Prompt 付きモデル作成 (推奨)
ollama create gemma3-sp -f models/Modelfile.gemma3-sp
# 日本語特化サブモデル (オプション)
# swallow:8b-v0.5 は HuggingFace GGUF → Modelfile 経由で取り込み

# 動作確認
ollama run gemma3-sp "あなたは誰ですか？"
```
...
- gemma3 がテキスト・Vision 両方を担うため、追加の Vision モデル不要
...
### ロードマップ外の検討項目 (Backlog)

#### A. 設計改善 (FT前にコードで解決すべき)

ベースライン評価 (`evals/reports/baseline_20260409.md`) と replan評価 (`evals/reports/replan_20260409.md`) で発見された、**プロンプト調整では解決しない構造的課題**。FT より先にコードで対処する。

| 項目 | 優先度 | 由来 | 概要 |
|---|---|---|---|
| ~~**構造化出力の強制 (Ollama `format=json`)**~~ | ~~高~~ → 解決済 | m1/m4/rp6 JSON崩れ | Ollama API の `format=json` パラメータで JSON 構文を強制する方式で解決 |
| ~~**code 引数のファイルパス化**~~ | ~~高~~ → 解決済 | m1/m4 JSON崩れ | 先に file_ops で `.py` を書き、code_executor は path を受ける形に変更済み |
| ~~**Verifier コンポーネント独立化**~~ | ~~中~~ → 解決済 | replan rp4 偽done | Observerから「結果の妥当性検証」責務を分離して `Verifier` クラスとして実装済み (§5.3.1 参照) |
| ~~**MemorySearchTool 化**~~ | ~~中~~ → 解決済 | Phase 2 検討事項 | 3層メモリの L3 検索を `BaseTool` 化し、Planner が必要時のみ明示呼び出しできるように実装済み |
| **専用OCRツール (Tesseract / PaddleOCR)** | 中 | Phase 2 vision検討 | Gemma 3 のネイティブビジョンでは長文・表・日本語縦書き・手書きの精度が落ちる。`BaseTool` として OCR エンジンを追加し、VisionAdapter から委譲できるように |
| ~~**WhisperAdapter (音声入力)**~~ | ~~中~~ → 解決済 | README Phase 3 予定 | faster-whisper (GPU対応、34倍速) で音声→テキスト変換を実装済み |
| **FastAPI バックエンド + Web UI** | 中 | README Phase 2 Week 7-8 予定 | WebSocket でエージェント実行をストリーミング、React UI から操作 |

#### B. FT (QLoRA / DPO) で学ばせる項目

ベースライン評価で **プロンプト調整では届かないと確認された LLM 内部の振る舞い**。設計改善後も残る本質的な弱点。

| 項目 | 優先度 | 現状 | 学習データ |
|---|---|---|---|
| ~~**自己同一性の訂正**~~ | ~~高~~ → **解決済** | ~~k3 (「Gemini です」と誤答)~~ → **Ollama Modelfile の SYSTEM プロンプトで完全解決** (FT不要) | - |
| ~~**「知らない」と言える訓練**~~ | ~~高~~ → **解決済** | ~~k4 (架空イベントで人名捏造)~~ → **同上、SYSTEM プロンプトの「知らないことは知らないと素直に答えてください」で解決** (FT不要) | - |
| **「指摘事項なし」と言える訓練 (Yes-bias 抑制)** | 高 | c3 (バグなしコードに無理矢理修正)。**System Prompt では解決しない** (ユーザーが「バグを指摘して」と言う指示力がSystem Promptを上回る)。DPO データ 15ペア作成済み (`evals/sft_curated/dpo_honesty.yaml`)。 | DPO chosen/rejected ペア 50〜200件 |
| **Plannerの構造化出力安定化** | ~~中~~ → **解決済** | format=json + code_executor path mode で JSON崩れ完全消滅 (FT不要) | - |
| **指示の字面遵守** | 中 | rp4 (math.pi 段階の省略), rp6 (DL試行を echo で代替) | 自作 (順序指定タスクの正解例) 100〜300件 |

> **FT/DPO 実施記録 (2026-04-09〜10)**
>
> - **SFT (QLoRA)**: unsloth で Gemma 3 12B に 69件 SFT を実施。学習自体は成功 (loss 1.83→0.64、341秒) したが、GGUF 変換で unsloth 内部バグ (`# of LoRAs = 417 does not match # of saved modules = 0`) が発生。llama.cpp の `convert_hf_to_gguf.py` で Q8_0 (12GB) に変換し Ollama 取り込みまで完了したが、**FT効果はほぼゼロ** (k3/k4/c3 全て未改善)。原因: データ量不足 (69件では 12B パラメータに影響しない)、Gemma 3 自身での増強データが自分のバイアスを再生産していた。
>
> - **DPO**: transformers 5.5 + peft + trl 1.0 で Gemma 3 12B の DPO を試みたが、**Gemma 3 の `token_type_ids` 要求と trl 1.0 の非互換** (`ValueError: token_type_ids is required as a model input when training`) で実行できず。tokenizer 側で `model_input_names` から除外しても、Gemma 3 の `modeling_gemma3.py` 内の `create_causal_mask_mapping` が直接要求するため回避不能。unsloth 経由でも依存地獄 (triton `AttrsDescriptor` 不在、torch CPU/CUDA 入れ替え等) で断念。
>
> - **結論**: k3/k4 は System Prompt のみで解決。c3 (Yes-bias) は **Gemma 3 + DPO の互換性が安定するまで保留**。代替として Planner/Chat の System Prompt 強化 + Verifier での検知で実用回避する。DPO データ (`evals/sft_curated/dpo_honesty.yaml`, 15ペア) は将来の再挑戦用に保存済み。

#### C. インテグレーション・運用

| 項目 | 優先度 | 概要 |
|---|---|---|
| ~~**MCP (Model Context Protocol) 対応**~~ | ~~高~~ → 解決済 | stdio transport の MCP サーバーを `mcp_server.py` として実装済み。ツールを MCP 経由で外部クライアントに公開可能 |
| **学習データ自動生成パイプライン** | 高 | エピソード記憶 (ChromaDB) を SFT データセットへ変換するスクリプト群。FT サイクルの自動化 |
| **VSCode 拡張** | 中 | エディタ統合 |
...
# Claude Desktop 等の MCP クライアントから接続する場合は
...
Phase 1   (Python、テキストのみ)
    ↓
Phase 2   (Python + gemma3 ネイティブビジョン、画像対応)
    ↓
Phase 3   (Python + faster-whisper + Rust拡張、全モーダル対応)
    ↓
...
| v0.10.0 | 2026-04 | LLM を gemma3-sp (Gemma 3 12B + System Prompt) に統一、swallow:8b-v0.5 (日本語特化サブ) 追加、Vision を gemma3 ネイティブビジョンに移行 (llava 廃止)、音声認識を faster-whisper (GPU対応) に移行、Verifier コンポーネント追加 (§5.3.1)、eval/training/MCP サーバーをディレクトリ構成に追加、ロードマップ進捗を実態に合わせて更新、拡張計画の完了項目を反映 |
...
class VisionAdapter(BaseInputAdapter):
    async def to_text(self, input_data: Any) -> str:
        ...
        return await self._gemma3_describe(image_bytes)
```
...
| gemma3 Vision モデル未起動 | L3 | Vision モデル未ロード | テキストのみモードで継続 + ユーザー通知 |
