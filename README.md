...
| 項目 | 優先度 | 由来 | 学習データ |
|---|---|---|---|
| **自己同一性の訂正** | 高 | k3 (「Gemini です」と誤答) | システムプロンプト + 50〜100件のSFTデータ |
| **「知らない」と言える訓練** | 高 | k4 (架空イベントで人名捏造) | TruthfulQA 系 + 自作架空質問 200〜500件 |
| **「指摘事項なし」と言える訓練 (Yes-bias 抑制)** | 高 | c3 (バグなしコードに無理矢理修正) | 自作 (バグなしコード→「問題なし」) 100〜300件 |
| **Plannerの構造化出力安定化** | 中 | 全評価 (設計改善で大半は潰せる見込み) | 本リポジトリの実エピソードから自動生成 500〜1000件 |
